...
class CSVDatasetConverter:
    ...
    def create_training_format(self, input_text, output_text, feedback_text):
        ...
        return {
            "input": input_text,      # Text with errors
            "output": combined_output # Correction + Feedback
        }
    ...
    def process_csv_file(self, csv_file):
        ...
        try:
            ...
            for _, row in df.iterrows():
                input_text = self.clean_text(row['Input'])           # Text with errors
                output_text = self.clean_text(row['Output'])         # Corrected text
                feedback_text = self.clean_text(row['Updated Feedback'])  # CBC feedback
